...
import { getAllAppliedLoansOfUser, getAppliedLoans, getAppliedLoansStatus } from "@/lib/Loans";
...
const RejectedLoans = () => {
  ...
  const fieldsToDisplayForCSV = ["id" , "loanTypeId" , "customerId" , "aadhar" , "pan" , "additionalDocument" , "loanAmount" , "address" , "durationMonths" , "emiAmount" , "reasonForRejection"];

  // Fetch all loan types to resolve names
  ...
  const fetchRejectedLoans = async () => {
    try {
      const filterParams = {loanStatus : "rejected"}
      ...
      setRejectedLoans(enrichedLoans);
    } catch (error) {
      console.error("Failed to fetch rejected loans:", error);
      alert("Failed to fetch rejected loans.");
    }
  };
  ...
  const tableData = rejectedLoans.map((loan, index) => (
    <tr
      key={loan.id || index}
    ...
    >
      {fieldsToDisplay.map((keyName, cellIndex) => (
        ...
        >
          {keyName.key === "id" ? index + 1 : loan[keyName.key]}
        </td>
      ))}
  ...
  return (
    <div className="max-w-5xl mx-auto my-8 p-6 bg-white rounded-lg shadow-lg">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-semibold text-blue-700">Rejected Loans</h2>
